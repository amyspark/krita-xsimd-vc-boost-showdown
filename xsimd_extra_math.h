...
#ifndef XSIMD_ADDITIONAL_MATH_H
#define XSIMD_ADDITIONAL_MATH_H

#include <xsimd/xsimd.hpp>

class XsimdExtraMath
{
public:
    ...
    static inline xsimd::batch<float, A> erf(const xsimd::batch<float, A> x)
    {
        ...
        xa = xsimd::select(precisionLimit, float_v(0), xa);
        float_v sign(1.0f);
        ...
        y = xsimd::select(precisionLimit, float_v(1), y);
        return sign * y;
    }
};
